...
w = 0.5;            % std dev of model noise
v = 0.01;              % std dev of measurement noise
q = 20;              % # ensemble members (simulation runs per time step)
...
%     y_meas(round(N*cell),1:end) = T_FD(solverRuns:solverRuns:end,round(N*cell));
      y_meas(round(N*cell),1:end) = 500; %T_FD(solverRuns:solverRuns:end,round(N*cell)) + 30.*ones(length(t)-1,1);
...
addpath('~/dataAssimilation/old_1DheatCond')  ;
